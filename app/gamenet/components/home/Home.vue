<template>
    <div class="community" @touchmove="touchmove" @touchend="touchmend">
        ...
        <!-- spinner 的四个属性 'default' 'circles' 'bubbles' 'spiral' -->
...
export default {
    ...
    created() {
        ...
        if( this.isRefreshHomeData ) this.clearHomeData();      //通过是否发表话题给的状态判断是否清除缓存数据
    },
    mounted(){
        ...
        CbHomeData: function(res){          //有uid回调的接口数据
            ...
            if (res.code == 1) {            // 如果查询结果为真
                ...
                if (res.data.tz.length) {       // 如果有数据则进入将新的数据与老的数据拼接
                    if(this.form.page==1){
                        this.setHomeData({data:res.data,type:'refresh',page:this.form.page});
                        this.$refs.infiniteLoading.$emit('$InfiniteLoading:reset'); //重置组件处于最初状态
                    }else{
                        this.setHomeData({data:res.data,type:'loadmore',page:this.form.page});
                    }
